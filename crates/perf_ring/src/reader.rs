...
mod tests {
    ...
    fn test_ring_reader() {
        ...
        let mut ring1 = unsafe { PerfRing::init_contiguous(&mut data1, n_pages, page_size).unwrap() };
        let mut ring2 = unsafe { PerfRing::init_contiguous(&mut data2, n_pages, page_size).unwrap() };

        // Test that adding a ring while active fails
        reader.start().unwrap();
        assert!(matches!(reader.add_ring(unsafe { 
        ...
        assert!(reader.is_empty());
        assert!(matches!(reader.peek_timestamp(), Err(RingReaderError::NotActive)));
        assert!(matches!(reader.current_ring(), Err(RingReaderError::NotActive)));
        ...
        for (i, &expected) in expected_timestamps.iter().enumerate() {
            ...
            assert!(idx < reader.rings.len(), "Ring index {} out of bounds [0, {})", idx, reader.rings.len());
            ...
            assert_eq!(size, expected_size, "Expected size {}, got {}", expected_size, size);
            
            let mut ring_data = vec![0u8; expected_ring_data[i].len()];
            ring.peek_copy(&mut ring_data, 4).unwrap();
            
            assert_eq!(&ring_data[..], expected_ring_data[i], 
                "Expected ring data {:?}, got {:?}", expected_ring_data[i], ring_data);
            
            reader.pop().unwrap();
        }
        
        // Should be empty after reading all events
        assert!(reader.is_empty(), "Expected reader to be empty after reading all events");
    ...
    fn test_lost_records() {
        ...
        assert_eq!(ring.peek_type(), PERF_RECORD_SAMPLE, "Expected PERF_RECORD_SAMPLE");
        reader.pop().unwrap();
        ...
        let (ring, idx) = reader.current_ring().unwrap();
        assert_eq!(idx, 0, "Expected ring index 0, got {}", idx);
        assert_eq!(ring.peek_type(), PERF_RECORD_LOST, "Expected PERF_RECORD_LOST");
        reader.pop().unwrap();
        ...
        let ts = reader.peek_timestamp().unwrap();
        assert_eq!(ts, 100, "Expected timestamp 100 for normal event, got {}", ts);
